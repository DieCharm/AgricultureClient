...
export class CrudComponent implements OnInit, OnDestroy {
  ...
  onRedirect: Observable<string> = new Observable<string>();
  redirectSubscription: Subscription = new Subscription();

  constructor(public httpService: HttpService) { }

  ngOnInit(): void {
    this.redirectSubscription = this.onRedirect.subscribe((path) => {
      this.initializeTable(path);
    });
  }
  ...
  subscribeToForm(formComponent: any) {
    this.subscription = formComponent.onSubmit.subscribe(() => {
